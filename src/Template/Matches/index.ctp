...
<div class="matches index large-10 medium-9 columns">
    ...
    <?php foreach ($matches as $match): ?>
        <tr>
            <td><?= $match->has('home_team') ? $this->Html->link($match->home_club, ['controller' => 'Teams', 'action' => 'view', $match->home_team_id]) : ''?></td>
            <td><?= $match->has('away_team') ? $this->Html->link($match->away_club, ['controller' => 'Teams', 'action' => 'view', $match->away_team_id]) : ''?></td>
            <td>
